...
describe('Тесты для скрипта управления параграфами', () => {
  ...
  test('Тест 10.Должны удаляться самые старые параграфы', () => {
    ...
    const input = document.getElementById('textInput');
    const button = document.getElementById('addButton');
    const originalParagraphs = Array.from(document.querySelectorAll('p')).map(
      (p) => p.textContent
    );
    ...
    const currentParagraphs = Array.from(document.querySelectorAll('p')).map(
      (p) => p.textContent
    );
  ...
  test('Тест 13.Корректная работа при множественном добавлении', () => {
    ...
    expect(paragraphTexts).toEqual(
      expect.arrayContaining(['Третий', 'Четвертый', 'Пятый'])
    );
  });
  ...
  test('Тест 14.Новые параграфы должны иметь правильную структуру', () => {
    ...
    const newParagraph =
      document.querySelectorAll('p')[document.querySelectorAll('p').length - 1];
